...
export default function MovieGrid({ guessedMovies }: MovieGridProps) {
  ...
  return (
    <section
      className="rounded-lg border border-zinc-200 bg-gray-50 p-2 sm:p-4 dark:border-zinc-700 dark:bg-zinc-800"
      aria-label={`Movies guessed: ${guessedMovies.length} total`}
    >
      <ul
        ref={gridRef}
        className="grid grid-cols-3 gap-1.5 sm:gap-2 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 list-none m-0 p-0"
        aria-live="polite"
